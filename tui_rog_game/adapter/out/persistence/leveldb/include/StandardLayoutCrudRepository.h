    ...
    namespace Adapter {
        namespace Out {
            namespace Persistence {
                ...
                template <typename T>
                class StandardLayoutCrudRepository<T, std::enable_if_t<std::is_standard_layout_v<T>>> {
                public:
                    ...
                    void save(const std::string& key, const T& entity) {
                        ...
                        if (!provider.Put(lower_key, value)) {
                            spdlog::error("CrudRepository: Failed to save key '{}' immediately.", lower_key);
                        } else {
                            spdlog::debug("CrudRepository: Saved key '{}' of size {} immediately.", lower_key, sizeof(T));
                        }
                    }
                    ...
                    void saveForBatch(const std::string& key, const T& entity) {
                        ...
                        spdlog::debug("CrudRepository: Added Put for key '{}' to batch.", lower_key);
                    }
                    ...
                    std::optional<T> findById(const std::string& key) {
                        ...
                        if (value_str_opt->length() == sizeof(T)) {
                            ...
                            spdlog::error("CrudRepository: Value length mismatch for key '{}'. Expected {} bytes, got {} bytes.", lower_key, sizeof(T), value_str_opt->length());
                            return std::nullopt;
                        }
                    }
                    ...
                    void deleteForBatch(const std::string& key) {
                        ...
                        spdlog::debug("CrudRepository: Added Delete for key '{}' to batch.", lower_key);
                    }
