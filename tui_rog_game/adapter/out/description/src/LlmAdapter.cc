    ...
    namespace Adapter {
        namespace Out {
            namespace Description {
                ...
                std::string LlmAdapter::generateDescription(const Port::Out::GameStateDTO& game_state) {
                    const auto& player = game_state.player;
                    const auto& map = game_state.map;
                    const auto& player_position = player.getPosition();

                    spdlog::info("AI Adapter: Generating description for player at ({}, {})", player_position.x, player_position.y);
                    ...
                    std::string prompt = "당신은 던전 마스터입니다. 다음 게임 상태 정보를 바탕으로 플레이어 주변 상황을 한국어로 간결하고 생생하게 묘사해주세요. 묘사는 50단어 이내로 해주세요.\n\n";
                    
                    // 플레이어 정보 추가
                    prompt += "플레이어 정보: 레벨 " + std::to_string(player.getLevel()) + ", 경험치 " + std::to_string(player.getXp()) + ", 체력 " + std::to_string(player.getHp()) + "/" + std::to_string(player.getMaxHp()) + ".\n";
                    ...
                    if (!nearby_elements.empty()) {
                        prompt += "현재 위치 (" + std::to_string(px) + ", " + std::to_string(py) + ") 주변에는 ";
                        ...
                        prompt += "이(가) 있습니다.\n";
                        prompt += "몬스터가 가까이 있다면 위협적인 분위기를, 아이템이 가까이 있다면 흥미로운 발견을, 출구가 가까이 있다면 탈출 또는 다음 단계로의 기회를 암시하는 묘사를 포함해주세요.\n";
                    } else {
                        prompt += "현재 위치 (" + std::to_string(px) + ", " + std::to_string(py) + ") 주변에는 특별한 것이 없습니다.\n";
                    }
                    ...
                    auto res = cli_->Post(path.c_str(), headers, request_body.dump(), "application/json");
                    ...
                    if (res && res->status == 200) {
                        ...
                        spdlog::error("AI Adapter: Failed to get response from LLM API. Status: {}, Error: {}", 
                                     res ? res->status : 0, res ? res->body : "(no response body)");
