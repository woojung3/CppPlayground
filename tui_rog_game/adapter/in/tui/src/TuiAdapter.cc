...
void TuiAdapter::run() {
    ...
    auto renderer = Renderer([&] { // Capture this by reference for show_start_screen_
        if (show_start_screen_) {
            auto title = ftxui::text("TUI-ROG: AI Dungeon Master") | ftxui::bold | ftxui::center;
            auto instructions = ftxui::vbox({
                ftxui::text(""),
                ftxui::text("Controls:"),
                ...
                ftxui::text("  M: Toggle Description Model (Hardcoded/AI)"), // Added M instruction
                ftxui::text("  Q: Quit"),
                ftxui::text(""),
        ...
        auto render_map_elements = [&](const Domain::Model::Map& current_map, const Domain::Model::Player& current_player) {
            std::vector<Element> rows_elements;
            for (int y = 0; y < current_map.getHeight(); ++y) {
                Elements row_chars;
                for (int x = 0; x < current_map.getWidth(); ++x) {
                    if (current_player.getPosition().x == x && current_player.getPosition().y == y) {
        ...
        auto render_ascii_art = [&]() -> Element {
            ...
            for (const auto& pos : adjacent) {
                if (pos.first >= 0 && pos.first < map.getWidth() && 
                    pos.second >= 0 && pos.second < map.getHeight()) {
        ...
        auto ascii_art_view = vbox({
            text("Nearby") | ftxui::bold | ftxui::center,
            separator(),
        ...
        auto stats_view = vbox(Elements{
            text("Player Stats") | ftxui::bold,
            separator(),
            text("HP: " + std::to_string(player.getHp()) + "/" + std::to_string(player.getMaxHp())),
            ...
            text("Vit: " + std::to_string(player.getStats().vitality)),
            separator(),
            text("Inventory:") | ftxui::bold,
            vbox([&] {
                Elements inventory_elements;
                            std::transform(player.getInventory().begin(), player.getInventory().end(), std::back_inserter(inventory_elements),
        ...
        auto mini_map_view = render_map_elements(map, player) | border | ftxui::size(ftxui::HEIGHT, ftxui::EQUAL, map.getHeight() + 2); // +2 for border

        // Message Log (bottom-right, fixed height)
        Elements log_lines;
        int start_index = std::max(0, (int)message_log_.size() - 5); // Display last 5 messages
        for (size_t i = start_index; i < message_log_.size(); ++i) {
            log_lines.push_back(paragraph(message_log_[i])); // Use paragraph for word wrapping
        }
        ...
        auto top_panel = hbox(Elements{
            ascii_art_view,
            separator(),
            stats_view | ftxui::size(ftxui::WIDTH, ftxui::EQUAL, 30) // Fixed width for stats
        }) | flex;

        auto bottom_panel = hbox(Elements{
            mini_map_view | ftxui::size(ftxui::WIDTH, ftxui::EQUAL, map.getWidth() + 2), // Explicitly set minimap width
            separator(),
            message_log_view | flex // Message log takes flexible width in bottom panel
    ...
    auto component = CatchEvent(renderer, [&](Event event) {
        ...
        if (event.is_character()) {
            ...
            switch (input) {
                ...
                case 'x':
                    if (game_state_ptr_->has_value()) {
                        command = Port::In::PlayerActionCommand(Port::In::PlayerActionCommand::ATTACK, game_state_ptr_->value().player.getAttackPower());
                    } else {
                ...
                case 'u':
                    // This needs a way to get item name from user. For now, let's assume a hardcoded item for testing.
...
void TuiAdapter::render(const Port::Out::GameStateDTO& game_state, const std::vector<std::unique_ptr<Domain::Event::DomainEvent>>& events) {
    ...
    if (!game_state_ptr_->has_value() || 
        game_state_ptr_->value().player.getPosition().x != game_state.player.getPosition().x ||
        game_state_ptr_->value().player.getPosition().y != game_state.player.getPosition().y ||
    ...
    for (const auto& event : events) {
        if (event->getType() == Domain::Event::DomainEvent::Type::CombatStarted) {
        ...
        } else if (event->getType() == Domain::Event::DomainEvent::Type::PlayerAttacked) {
            const auto* player_attacked_event = dynamic_cast<const Domain::Event::PlayerAttackedEvent*>(event.get());
            if (player_attacked_event) {
                message_log_.push_back(player_attacked_event->toString());
            }
        } else if (event->getType() == Domain::Event::DomainEvent::Type::EnemyAttacked) {
            const auto* enemy_attacked_event = dynamic_cast<const Domain::Event::EnemyAttackedEvent*>(event.get());
            if (enemy_attacked_event) {
                message_log_.push_back(enemy_attacked_event->toString());
            }
        } else if (event->getType() == Domain::Event::DomainEvent::Type::EnemyDefeated) {
            const auto* enemy_defeated_event = dynamic_cast<const Domain::Event::EnemyDefeatedEvent*>(event.get());
            if (enemy_defeated_event) {
                message_log_.push_back(enemy_defeated_event->toString());
            }
        } else if (event->getType() == Domain::Event::DomainEvent::Type::PlayerLeveledUp) {
            const auto* player_leveled_up_event = dynamic_cast<const Domain::Event::PlayerLeveledUpEvent*>(event.get());
            ...
            spdlog::info("TuiAdapter: Received ItemUsedEvent. Type: {}", static_cast<int>(event->getType()));                              
            const auto* item_used_event = dynamic_cast<const Domain::Event::ItemUsedEvent*>(event.get());                                  
            if (item_used_event) {                                                                                                         
                spdlog::info("TuiAdapter: dynamic_cast to ItemUsedEvent successful. Item name: {}", item_used_event->getItemName());       
                message_log_.push_back("You used a " + item_used_event->getItemName() + "!");                                              
            } else {                                                                                                                       
                spdlog::warn("TuiAdapter: dynamic_cast to ItemUsedEvent failed for event type {}.", static_cast<int>(event->getType()));
            }                                                                                                                              
        } else if (event->getType() == Domain::Event::DomainEvent::Type::DescriptionGenerated) {
            const auto* description_generated_event = dynamic_cast<const Domain::Event::DescriptionGeneratedEvent*>(event.get());
