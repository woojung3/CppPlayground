    ...
    switch (command.type) {
            ...
            break;
                case TuiRogGame::Port::In::PlayerActionCommand::ATTACK: {
                    ...
                    if (!current_enemy_) { // If not already in combat
                        spdlog::debug("GameEngine: Not in combat, checking for adjacent enemies.");
                        ...
                        for (const auto& adj_pos : adjacent_positions) {
                            spdlog::debug("GameEngine: Checking adjacent position ({}, {}).", adj_pos.x, adj_pos.y);
                            if (map_->isValidPosition(adj_pos.x, adj_pos.y)) {
                                if (auto enemy_opt = map_->getEnemyAt(adj_pos)) {
                                    current_enemy_ = std::ref(enemy_opt->get());
                                    spdlog::debug("GameEngine: Enemy found at ({}, {}): {}. Setting current_enemy_.", adj_pos.x, adj_pos.y, current_enemy_->get().getName());
                                    ...
                                    spdlog::info("Combat started with adjacent enemy {} at ({}, {}).", current_enemy_->get().getName(), adj_pos.x, adj_pos.y);
                                    enemy_found = true;
                        ...
                        spdlog::debug("GameEngine: Already in combat with {}. Proceeding with attack.", current_enemy_->get().getName());
                    }
        
                    // Player attacks enemy
                    int player_damage = player_->getAttackPower();
                    spdlog::debug("GameEngine: Player attacking {} for {} damage.", current_enemy_->get().getName(), player_damage);
                    current_enemy_->get().takeDamage(player_damage);
                    events.push_back(std::make_unique<Domain::Event::PlayerAttackedEvent>(
                        player_damage, current_enemy_->get().getName(), current_enemy_->get().getHealth()));
                    spdlog::info("Player attacked {} for {} damage. {}'s health: {}",
                                 current_enemy_->get().getName(), player_damage, current_enemy_->get().getName(), current_enemy_->get().getHealth());
        
                    if (current_enemy_->get().getHealth() <= 0) {
                        spdlog::debug("GameEngine: Enemy {} defeated.", current_enemy_->get().getName());
                        // Enemy defeated
                        ...
                        spdlog::info("{} defeated! Player gained {} XP.", current_enemy_->get().getName(), xp_gained);
                        ...
                        spdlog::debug("GameEngine: Enemy {} still alive. Health: {}. Enemy attacking player.", current_enemy_->get().getName(), current_enemy_->get().getHealth());
        ...
        std::vector<std::unique_ptr<TuiRogGame::Domain::Event::DomainEvent>> GameEngine::processPlayerMove(int dx, int dy) {
            ...
            if (auto enemy_opt = map_->getEnemyAt(new_pos)) {
                ...
                spdlog::debug("GameEngine: Enemy encountered at ({}, {}): {}. Setting current_enemy_.", new_pos.x, new_pos.y, current_enemy_->get().getName());
                events.push_back(std::make_unique<Domain::Event::CombatStartedEvent>(
                    current_enemy_->get().getTypeName(), // Use getTypeName()
                    current_enemy_->get().getName(),
                    current_enemy_->get().getHealth(),
                ...
                spdlog::info("GameEngine: CombatStartedEvent created with enemy {} at ({}, {}).", current_enemy_->get().getName(), new_pos.x, new_pos.y);
