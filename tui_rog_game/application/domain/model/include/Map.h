...
public:
    ...
    void generate();
    Position getStartPlayerPosition() const { return start_player_position_; } // New method
    ...
    const std::map<Position, std::unique_ptr<Enemy>>& getEnemies() const { return enemies_; }
    const std::map<Position, std::unique_ptr<Item>>& getItems() const { return items_; }

    // Setters for persistence
    void setTiles(std::vector<std::vector<Tile>> tiles) { tiles_ = std::move(tiles); }
...
private:
    // void addEnemy(Position position, std::unique_ptr<Enemy> enemy); // Moved to public
    // void addItem(Position position, std::unique_ptr<Item> item); // Moved to public
